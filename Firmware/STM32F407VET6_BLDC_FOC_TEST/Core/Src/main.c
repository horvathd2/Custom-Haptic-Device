...
float current_offset = 1.65f;     // in volts
float current_sensitivity = 0.06f;  // V/A
...
void HAL_ADCEx_InjectedConvCpltCallback(ADC_HandleTypeDef *hadc)
{
	if(hadc->Instance == ADC1)
	{
		...
		raw_adc_curr = (uint16_t)ADC1->JDR1; //maybe use atomic or disable interrupts
		//ib = ADC2->JDR1;
		//ic = ADC3->JDR1;
	}
}
...
void current_calibrate(void)
{
	...
	// Make sure motor is off / no current!
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 0);
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 0);
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_3, 0);

	osDelay(50); // wait for current to settle

	for(int i = 0; i < samples; i++)
	{
		__disable_irq();
		uint16_t raw = raw_adc_curr;
		__enable_irq();

		sum += (raw * 3.3f) / 4095.0f;   // use 3.3V
		osDelay(1);
	}

	current_offset = sum / samples;
	// Print it via USB
	char buf[64];
...
/**
 * @brief  Function implementing the mainTask1 thread.
 * @param  argument: Not used
 * @retval None
 */
/* USER CODE END Header_main_task1 */
void main_task1(void *argument)
{
	...
	while(1)
	{
		...
		float adc_vref = 3.1f;                    // keep this (or 2.85–2.87)
		//float current_offset = 1.650f;

		// === Stronger moving average ===
		static float current_filtered = 0.0f;
		const float alpha = 0.1f;                 // lower = smoother (try 0.05–0.12)

		float adc_voltage = (raw_adc_curr_local * adc_vref) / 4095.0f;
		float current_raw = (adc_voltage - current_offset) / 0.06f;
